...
import { fetchAsanaData } from '../api'
...
export default function ChatPanel({ messages, onSend, loading, hasHtml, activeJsonName, onClearJson }) {
  ...
  const handleAsanaImport = async () => {
    try {
      toast.loading('Fetching Asana data…', { id: 'asana' })
      const data = await fetchAsanaData()
      setJsonData(data)
      setJsonFileName('asana-data.json')
      toast.success('Asana data loaded!', { id: 'asana' })
    } catch (err) {
      const detail = err?.response?.data?.detail || 'Failed to fetch Asana data'
      const isNotConnected = err?.response?.status === 400 && detail.includes('Settings')
      toast(
        isNotConnected
          ? '⚙️ Asana not connected — add your PAT in Settings first'
          : detail,
        { id: 'asana', icon: isNotConnected ? null : '❌', duration: 4000 }
      )
    }
  }
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 select-none gap-3">
            ...
            <div className="text-xs text-gray-600 space-y-1 max-w-xs">
              ...
              <p>• "Import from Asana" to build a project dashboard</p>
            </div>
          </div>
        )}
        {messages.map((msg, i) => (
      ...
      {/* New JSON about to be sent this message */}
      {jsonFileName && (
        <div className="mx-4 mb-1 flex items-center gap-2 bg-brand-500/10 border border-brand-500/30 rounded-lg px-3 py-1.5 text-xs text-brand-500">
          ...
          <button onClick={() => { setJsonData(null); setJsonFileName('') }} className="hover:text-red-400">✕</button>
        </div>
      )}

      {/* Persistent JSON — active until new JSON is uploaded or dashboard is saved */}
      {!jsonFileName && activeJsonName && (
        <div className="mx-4 mb-1 flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-xs text-gray-400">
          ...
          <span className="truncate flex-1">JSON active — {activeJsonName.replace('(using uploaded JSON)', 'reusing uploaded data')}</span>
          <button
            onClick={onClearJson}
            title="Clear JSON"
